...
func (c *TradeRecords) GetPendingRefunds(req types.GetPendingRefundsReq) (r []types.GetRefundInfo, total int64, err error) {
	query := c.DB.Model(&model.TradeRecords{}).
		Joins("left join users as seller on seller.userID = trade_records.sellerID").
		Joins("left join users as buyer on buyer.userID = trade_records.buyerID").
		Joins("left join goods on goods.goodsID = trade_records.goodsID").
		Where("trade_records.status = ?", "退款中")

	if req.SearchQuery != "" {
		query = query.Where("trade_records.tradeID = ?", req.SearchQuery)
	}
	...
	err = query.Offset((req.PageNum - 1) * req.PageSize).Limit(req.PageSize).
		Select("trade_records.tradeID as TradeID," +
			"goods.goodsName as GoodsName," +
			"trade_records.turnoverAmount as Price," +
			"trade_records.shippingCost as ShippingCost," +
			"seller.userName as SellerName," +
			"trade_records.sellerReason as SellerReason," +
			"buyer.userName as BuyerName," +
			"trade_records.buyerReason as BuyerReason," +
			"trade_records.sellerID as SellerID," +
			"trade_records.buyerID as BuyerID," +
			"trade_records.orderTime as OrderTime," +
			"trade_records.payTime as PayTime," +
			"trade_records.refundTime as RefundTime," +
			"trade_records.shippingTime as ShippingTime," +
			"trade_records.turnoverTime as TurnoverTime," +
			"trade_records.status as Status").
		Scan(&refunds).Error
	...
	for _, refund := range refunds {
		r = append(r, types.GetRefundInfo{
			TradeID:      refund.TradeID,
			GoodsName:    refund.GoodsName,
			Price:        refund.Price,
			ShippingCost: refund.ShippingCost,
			SellerName:   refund.SellerName,
			SellerReason: refund.SellerReason,
			BuyerName:    refund.BuyerName,
			BuyerReason:  refund.BuyerReason,
			SellerID:     refund.SellerID,
			BuyerID:      refund.BuyerID,
			OrderTime:    refund.OrderTime.Format("2006-01-02 15:04:05"),
			PayTime:      refund.PayTime.Format("2006-01-02 15:04:05"),
			RefundTime:   refund.RefundTime.Format("2006-01-02 15:04:05"),
			ShippingTime: refund.ShippingTime.Format("2006-01-02 15:04:05"),
			TurnoverTime: refund.TurnoverTime.Format("2006-01-02 15:04:05"),
